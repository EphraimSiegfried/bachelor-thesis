...
The Repository module mainly performs Git operations using the Git2 library. #footnote[https://github.com/rust-lang/git2-rs] Git2 is a Rust wrapper of the C++ libgit implementation, which provides low-level Git operations. #footnote[https://libgit2.org/] The Repository module gives abstractions over common Git operations used for the binary cache, e.g. creating commit objects with constant author, message and date values.
...
The Narinfo module constructs a Narinfo data structure from the Nix object metadata retrieved from the Nix daemon. It is also able to encode this metadata as a string, which is then stored as a blob in the Git database. Additionally, it signs the fingerprint of the Narinfo and appends it to the Narinfo (See @binary-cache-interface).
