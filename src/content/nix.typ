...
There are two primary methods for computing the hash. The standard approach is to hash information contained in the derivation associated with the store object. A derivation is a file specifying the build recipe and the complete graph of dependencies; essentially, this hashes the inputs. @nixdev-input-address There is also an experimental feature where the hash is computed directly from the final contents of the built object itself. @nixdev-content-address The former scheme is known as input-addressing, and the latter as content-addressing.
