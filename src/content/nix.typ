...
Package authors can also publish packages on a private registry and publish artifacts on a custom binary cache such  as Cachix. #footnote[https://www.cachix.org] #todo[say what cachix is] The benefit of using the official registry is that they will be made available at the official cache, which is set as trusted and available in every Nix installation.
