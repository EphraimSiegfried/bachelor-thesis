...
Package authors can also publish packages on a private registry and publish artifacts on a custom binary cache such as #link("https://www.cachix.org")[Cachix]. The benefit of using the official registry is that they will be made available at the official cache, which is set as trusted and available in every Nix installation.
...
Package contents are served in the NAR archive format. This format closely follows the abstract specification of a file system object tree @nixdev-nar.
