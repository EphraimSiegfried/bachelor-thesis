...
There are two primary methods for computing the hash. The standard approach is to hash information contained within a object's derivation, known as input-addressing. A derivation is a file specifying the build recipe and the complete graph of dependencies. @nixdev-input-address There is also an experimental feature called content-addressing, where the hash is computed directly from the final contents of the built object itself. @nixdev-content-address 
...
#figure(image("../diagrams/nix-pipeline.drawio.pdf", width: 80%), caption: "Nix Deployment Pipeline") <nix-pipeline>
...
The main purpose of the Nix daemon is for multi-user Nix installations. In this mode the Nix store is owned by some privileged user to prevent other users to manipulate the Nix store in a malicious way (e.g. install a Trojan horse). When untrusted users use the Nix CLI, control is forwarded to the Nix daemon which is run under the owner of the Nix store. In this scenario, commands to the Nix daemon are dispatched through interprocess communication via a socket located at `/nix/var/nix/daemon-socket/socket`. @nixdev-multi-user
