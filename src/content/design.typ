...
#figure(image("../diagrams/nix-filesystem.drawio.pdf", width: 40%), caption: "Nix Store. The arrows indicate dependencies between packages.") <nix-filesystem>
...
The current algorithm of adding a package closure is displayed in @add-closure-algo. The algorithm receives the Nix store hash as an argument. It returns the commit ID associated with that package if it is able to fetch the closure of the package. If it is not able to do so, it returns _NIL_. It tries to recursively add package contents to the Git repository. At its core, it is similar to a depth first search algorihm (See lines 24-34 in @add-closure-algo). The algorithm iterates through a package's direct dependencies (line 26). For  each dependency $d$, it makes a recursive call to _add_closure(d)_ (line 28). This means it fully processes one dependency, including all its dependencies, before moving to the next direct dependency in the list. Once it has collected all commit hashes of the dependencies, it constructs a commit using the hash collection as parent commits (line 35). 
